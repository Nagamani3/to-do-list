...
function showtask(){
    ...
    taskObj.forEach((item, index) => {
        ...
        html += `<tr>
                    <th scope="row" id ="text">${index+1 }</th>
                    ${taskCompleteValue}
                    <td><button type="button" id = "edit" onclick="edittask(${index})" class="text-primary"><i class="fa fa-edit"></i>Edit</button></td>
                    
                    <td><button type="button"id = "delete" onclick="deleteitem(${index})" class="text-danger"><i class="fa fa-trash"></i>Delete</button></td>
                </tr>`;
    });
...
//complete task
 function completetask(index){
    let webtask = localStorage.getItem("localtask");
    let taskObj = JSON.parse(webtask);
    taskObj[index] = '<span style="background-color:red">' + taskObj[index] + '</span>';
    let addedtasklist = document.getElementById("addedtasklist");
